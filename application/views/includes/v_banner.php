<section id="herobanner" class="wow fadeIn text-center">
  <!-- <div id="mobile-title">
    <h1>Welcome to Kelurahan Siwa</h1>
    <h2>Website informasi dan pelayanan Kelurahan Siwa</h2>
  </div> -->
  <div class="hero-container">
    <div id="carouselExampleSlidesOnly" class="carousel slide" data-ride="carousel">
      <div class="carousel-inner">
        <div class="carousel-item active">
          <div class="carousel-caption d-md-block" style="margin-bottom: 140px;">
            <h1 style="
  -webkit-text-stroke-width: 1px;
  -webkit-text-stroke-color: white;">SELAMAT DATANG DI KELURAHAN SIWA</h1> 
            <!-- <h2 style="color:white;" class="">Website informasi dan pelayanan Kelurahan Siwa</h2> -->
            <!-- <a href="<?=base_url("akun/masuk")?>" class="btn-get-started bg-dark scrollto">Get Started</a> -->
          </div>
          <img class="img-slide" style="width:100vw; height:100vh; opacity:0.8;" src="<?=base_url()?>assets/img/siwa.jpg" alt="First slide">
        </div>
        <!-- <div class="carousel-item">
          <div class="carousel-caption d-md-block" style="margin-bottom: 150px;">
            <h1>Welcome to Desa Pagerngumbuk</h1>            
            <a href="#layanan" class="btn-get-started scrollto">Get Started</a>
          </div>
          <img class="img-slide" src="<?=base_url()?>assets/img/bannersawahh1.jpg" alt="First slide">
        </div> -->
      </div>
    </div>
  </div>
</section>
